...
class RecommendationGenerator {
    constructor(apiKeys = {}) {
        this.hf = apiKeys.huggingface ? new inference_1.HfInference(apiKeys.huggingface) : null;
        this.openai = apiKeys.openai ? new openai_1.default({ apiKey: apiKeys.openai }) : null;
     ...
     */
    async generateWithHuggingFace(context) {
        if (!this.hf) {
...
Generate 3-5 specific, actionable recommendations that address the primary factor and current risk level. Focus on practical steps the user can take immediately or in the next few days.
     ...
     */
    parseHuggingFaceResponse(response, context) {
        // Extract meaningful sentences from the generated text
        const doc = (0, compromise_1.default)(response);
     ...
     */
    determineCategory(text, context) {
     ...
     */
    determinePriority(text, context) {
     ...
     */
    getFactorSpecificRecommendation(primaryFactor) {
        const recommendations = {
            'check-in': {
     ...
     */
    getFactorDescription(primaryFactor) {
