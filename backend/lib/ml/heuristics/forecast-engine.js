...
const recommendation_generator_1 = require("../nlp/recommendation-generator");
...
class BurnoutForecastEngine {
    ...
    async computeForecast(userId, signals, config) {
        ...
        const recommendations = await this.generateRecommendations(riskLevel, factors, overallScore, signalScores, this.generateEmotionalWeather(overallScore, trend), this.identifyPrimaryFactor(signalScores), trend);
    ...
    calculateConfidence(signalScores, config) {
        ...
        const recentSignals = signalScores.filter(s => s.signal.timestamp >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Last 7 days
        );
    ...
    async generateRecommendations(riskLevel, factors, overallScore, signalScores, emotionalWeather, primaryFactor, trend) {
        const recommendationGenerator = new recommendation_generator_1.RecommendationGenerator();
     ...
     */
    identifyPrimaryFactor(signalScores) {
     ...
     */
    generateFactorInsights(category, impact, signalScores) {
    ...
    analyzeCheckInFactor(signals, impact) {
        const recentSignals = signals.filter(s => s.signal.timestamp >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000));
    ...
    analyzeTaskFactor(signals, impact) {
        const lowCompletionTasks = signals.filter(s => s.signal.metadata?.completionRate === '0-19%' ||
            s.signal.metadata?.completionRate === '20-39%');
    ...
    analyzeCalendarFactor(signals, impact) {
        const highStressMeetings = signals.filter(s => s.signal.metadata?.meetingFrequency === '9+' ||
            s.signal.metadata?.meetingDuration === 'very-long');
        if (highStressMeetings.length > 0) {
            return {
        ...
        const moderateMeetings = signals.filter(s => s.signal.metadata?.meetingFrequency === '6-8');
    ...
    analyzeSleepFactor(signals, impact) {
        const poorSleepSignals = signals.filter(s => s.signal.metadata?.sleepDuration === 'less-than-6' ||
    ...
    analyzeActivityFactor(signals, impact) {
        const sedentarySignals = signals.filter(s => s.signal.metadata?.activityLevel === 'sedentary' ||
